...
class UserModel extends Model {
  ...
  Future<DocumentSnapshot> getUser(String userId) async {
    return await _firestore.collection(Constants.C_USERS).doc(userId).get();
  }
  ...
  Future<User> getUserObject(String userId) async {
    ...
    /// return user object
    return User.fromDocument(userDoc.data()! as dynamic);
  }
  ...
  Future<void> updateUserData(
    ...
    // Update user data
    _firestore.collection(Constants.C_USERS).doc(userId).update(data);
  }
  ...
  Future<void> updateUserDeviceToken() async {
    ...
    await _fcm.subscribeToTopic(Constants.NOTIFY_USERS);

    /// Update user device token
    /// Check token result
    if (userDeviceToken != null) {
      await updateUserData(userId: getFirebaseUser!.uid, data: {
        Constants.TOKEN: userDeviceToken,
      }).then((_) {
    ...
    if (getFirebaseUser != null) {
      ...
      await getUser(getFirebaseUser!.uid).then((userDoc) {
        ...
        if (userDoc.exists) {
          ...
          if (userDoc[Constants.STATUS] == 'blocked' ||
              userDoc[Constants.STATUS] == 'flagged') {
            // Go to blocked user account screen
            blockedScreen!();
          } else {
            // Update UserModel for current user
            updateUserObject(userDoc.data()! as dynamic);
  ...
  Future<void> signUp({
    ...
    await _firestore
        ...
        .set(<String, dynamic>{
      Constants.ID: this.getFirebaseUser!.uid,
      Constants.IMAGE: imageProfileUrl,
      Constants.NAME: name,
      Constants.SEX: sex,
      Constants.BIRTHDAY: birthday,
      Constants.PHONE_NUMBER: phone,
      Constants.EMAIL: email,
      Constants.AMOUNT: 0,
      Constants.PURCHASED_PRODUCTS: [],
      Constants.ORDER_PRODUCTS: [],
      Constants.ADDRESS:
          GHAddress(address: address, lat: lat, lng: lng, heading: heading),
      Constants.USER_STATUS: 'active',
      Constants.USER_LAST_LOGIN: FieldValue.serverTimestamp(),
      Constants.USER_REG_DATE: FieldValue.serverTimestamp(),
      Constants.USER_DEVICE_TOKEN: userDeviceToken,
    }).then((_) async {
      /// Get current user in database
  ...
  Future<void> flagUserProfile(
    ...
    await _firestore.collection(Constants.C_FLAGGED_USERS).doc().set({
      Constants.FLAGGED_USER_ID: flaggedUserId,
      Constants.FLAG_REASON: reason,
      Constants.FLAGGED_BY_USER_ID: this.user.id,
      Constants.TIMESTAMP: FieldValue.serverTimestamp()
    });
    // Update flagged profile status
    await this.updateUserData(
        userId: flaggedUserId, data: {Constants.USER_STATUS: 'flagged'});
  }
  ...
  Future<void> updateUserLocation({
    ...
    if (!isPassport) {
      ...
      final Placemark place =
          await getUserAddress(position.latitude, position.longitude);
      // Set values
      ...
      if (locationResult!.country != null) {
        country = locationResult.country!.toString();
      }

      // Check locality result
      if (locationResult.administrativeAreaLevel1 != null) {
        locality = locationResult.administrativeAreaLevel1.toString();
      } else {
    ...
    if (country != '') {
      ...
      await UserModel().updateUserData(userId: UserModel().user.id, data: {
        Constants.USER_GEO_POINT: geoPoint.data,
        Constants.USER_COUNTRY: country,
        Constants.USER_LOCALITY: locality
      });
  ...
  Future<void> updateProfileImage(
    ...
    final imageLink = await uploadFile(
        file: imageFile, path: uploadPath, userId: this.user.id);

    await updateUserData(userId: user.id, data: {Constants.IMAGE: imageLink});
  }
